...
def get_date() -> str:
    ...
    speak(f"Todays date and time is {now}")
    return now
...
if arduino is None:
    speak("Arduino is not connected. Alpha will still work without it.")
    status_label_text = "Arduino not connected."
else:
    status_label_text = "Arduino connected."

def check_arduino_connection():
    global arduino
    if arduino is None:
        new_conn = connect_arduino()
        if new_conn:
            arduino = new_conn
            speak("Arduino has been connected.")
            status_label.config(text="Arduino connected.", fg="#50fa7b")
            root.update()
    else:
        try:
            arduino.write(b'PING\n')  # harmless check to test connection
        except (serial.SerialException, OSError):
            arduino.close()
            arduino = None
            speak("Arduino has been disconnected.")
            status_label.config(text="Arduino disconnected.", fg="#ff5555")
            root.update()

    root.after(10000, check_arduino_connection)  # check every 10 seconds
...
status_label = tk.Label(main_frame, text=status_label_text, font=("Segoe UI", 12), bg="#232946", fg="#b8c1ec", wraplength=420, justify="center")
...
# Start periodic Arduino connection checks after GUI initializes
root.after(10000, check_arduino_connection)  # first check after 10 seconds
